...
export default function NotesPage() {
  ...
  useEffect(() => {
    const savedNotes = localStorage.getItem("focusnest-notes")
    if (savedNotes) {
      try {
  ...
  const handleSaveNote = (updatedNote: Note) => {
    ...
    setNotes(notes.map((note) => (note.id === noteWithSlug.id ? noteWithSlug : note)))
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      />
      <NoteEditor note={selectedNote} onSave={handleSaveNote} onDelete={handleDeleteNote} onExport={handleExportNote} />
    </div>
  )
}
