...
export default function PlannerPage() {
	...
	useEffect(() => {
		const savedPlans = localStorage.getItem("FocusMine-day-plans");
		if (savedPlans) {
			try {
	...
	useEffect(() => {
		localStorage.setItem("FocusMine-day-plans", JSON.stringify(dayPlans));
	}, [dayPlans]);
	...
	const getCurrentDayPlan = (): DayPlan => {
		...
		return (
			dayPlans.find((plan) => plan.date === dateKey) || {
				date: dateKey,
				blocks: [],
			}
		);
	...
	const updateDayPlan = (updatedPlan: DayPlan) => {
		setDayPlans((plans) => {
			const existingIndex = plans.findIndex(
				(plan) => plan.date === updatedPlan.date
			);
			if (existingIndex >= 0) {
	...
	const handleDeleteBlock = (blockId: string) => {
		const currentPlan = getCurrentDayPlan();
		const updatedBlocks = currentPlan.blocks.filter(
			(b) => b.id !== blockId
		);
		updateDayPlan({ ...currentPlan, blocks: updatedBlocks });
	};
	...
	const handleSaveBlock = (block: TimeBlock) => {
		const currentPlan = getCurrentDayPlan();
		const existingIndex = currentPlan.blocks.findIndex(
			(b) => b.id === block.id
		);

		let updatedBlocks: TimeBlock[];
		if (existingIndex >= 0) {
			updatedBlocks = currentPlan.blocks.map((b) =>
				b.id === block.id ? block : b
			);
		} else {
