...
interface ProjectContextType {
	...
	createProject: (
		project: Omit<Project, "id" | "createdAt" | "updatedAt" | "stats">
	) => void;
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
		const savedProjects = localStorage.getItem("FocusMine-projects");
		if (savedProjects) {
			try {
				const parsedProjects = JSON.parse(savedProjects).map(
					(project: any) => ({
						...project,
	...
	useEffect(() => {
		localStorage.setItem("FocusMine-projects", JSON.stringify(projects));
	}, [projects]);
	...
	const setActiveProject = (project: Project | null) => {
		...
		if (project) {
			localStorage.setItem("FocusMine-active-project", project.id);
		} else {
			localStorage.removeItem("FocusMine-active-project");
		}
	};
	...
	const deleteProject = (projectId: string) => {
		...
		// Remove project data
		localStorage.removeItem(`FocusMine-project-${projectId}`);
		...
		if (project) {
			const updatedProject: Project = {
				...
				stats: {
					...
					completedTasks: updatedData.tasks.filter(
						(task) => task.completed
					).length,
					totalPomodoros: updatedData.pomodoroSessions.filter(
						(s) => s.type === "pomodoro"
					).length,
					totalTimeSpent: updatedData.pomodoroSessions.reduce(
