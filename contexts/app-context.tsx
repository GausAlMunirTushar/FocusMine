  ...
  switch (action.type) {
    ...
    case "UPDATE_SETTINGS":
      ...
      return {
        ...
        timeLeft: state.isRunning ? state.timeLeft : getSessionDuration(state.currentSession, newSettings),
      }
    ...
    case "RESET_TIMER":
      return {
        ...state,
        isRunning: false,
        timeLeft: getSessionDuration(state.currentSession, state.settings),
      }
    ...
    case "COMPLETE_SESSION":
      const newCompletedPomodoros =
        state.currentSession === "pomodoro" ? state.completedPomodoros + 1 : state.completedPomodoros
...
function getSessionDuration(session: "pomodoro" | "shortBreak" | "longBreak", settings: Settings): number {
  switch (session) {
    case "pomodoro":
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const savedState = localStorage.getItem("focusnest-state")
    if (savedState) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("focusnest-state", JSON.stringify(stateToSave))
