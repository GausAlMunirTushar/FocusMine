...
export function PomodoroTimer({ embedded = false }: PomodoroTimerProps) {
  ...
  }, [state.isRunning, state.timeLeft, dispatch, state.settings.soundEnabled])
  ...
  useEffect(() => {
    if (state.settings.showInTitle && state.isRunning && !embedded) {
      ...
      document.title = `${timeString} - FocusNest`
    } else if (!embedded) {
      ...
      <Card className={cardClass}>
        <CardContent className={embedded ? "p-4 sm:p-6" : "p-8"}>
          <Tabs
            ...
            onValueChange={(value) => handleSessionChange(value as any)}
            className="w-full"
          >
            ...
            <div className="text-center">
              <div className={`font-mono font-bold mb-4 ${embedded ? "text-4xl sm:text-5xl" : "text-6xl"}`}>
                {formatTime(state.timeLeft)}
              </div>

              <p className={`text-muted-foreground mb-6 ${embedded ? "text-sm sm:text-base" : "text-lg"}`}>
                #{sessionCount} {getSessionMessage()}
              </p>

              <div className="flex gap-2 sm:gap-4 justify-center">
                {!state.isRunning ? (
                  <Button onClick={handleStart} size={embedded ? "default" : "lg"} className="px-4 sm:px-8">
                ...
                <Button onClick={handleReset} size={embedded ? "default" : "lg"} variant="outline">
      ...
      {!embedded && (
        <div className="mt-6 text-center">
          <p className="text-sm text-muted-foreground">
            {t("dashboard.todayPomodoros")}: {state.completedPomodoros}
          </p>
        </div>
      )}
    </div>
